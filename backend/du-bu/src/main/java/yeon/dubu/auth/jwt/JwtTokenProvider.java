...
import yeon.dubu.auth.config.ExpireTime;

import yeon.dubu.auth.dto.response.UserTokenResponseDto;
...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(@Value("${custom.jwt.secretKey}") String secretKey, UserRepository userRepository) {
    ...
    public UserTokenResponseDto.TokenInfo generateToken(Authentication authentication) {
        return generateToken(authentication.getName(), authentication.getAuthorities());
    }

    //name, authorities 를 가지고 AccessToken, RefreshToken 을 생성하는 메서드
    public UserTokenResponseDto.TokenInfo generateToken(String name, Collection<? extends GrantedAuthority> inputAuthorities) {
        //권한 가져오기
        ...
        return UserTokenResponseDto.TokenInfo.builder()
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        yeon.dubu.user.domain.User user = userRepository.findByEmail(email).orElseThrow(() -> new NoSuchUserException("토큰에 맞는 사용자 정보가 없습니다."));
        UserDetails principal = new User(claims.getSubject(), "", authorities);
        return new UsernamePasswordAuthenticationToken(user.getId(), accessToken, authorities);
